...
pub fn generate_elf64() {
    ...
    let data = b"\0.text\0.shstrtab\0";
    ...
    strstab_section_header.name = 4;
    strstab_section_header.offset = size_of::<Header>() as u64
        + (size_of::<SectionHeader>() as u64 * 5)
        + data.len() as u64
        + b"11".len() as u64
        + 24;
    strstab_section_header.size = b"_start\0".len() as _;
    buf.extend_from_slice(&strstab_section_header.to_bytes());

    buf.extend_from_slice(data);
    buf.extend_from_slice(b"11");
    buf.extend_from_slice(&[
        00, 00, 00, 00, 0x10, 00, 0x1, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,
        00, 00,
    ]);
    buf.extend_from_slice(b"_start\0");
